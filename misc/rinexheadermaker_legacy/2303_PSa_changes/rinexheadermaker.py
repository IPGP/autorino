...
def parseComment(lines,begin,numSection):
	debutComment = begin
	if numSection == 100 :
                expectedInfo ='^         Distance/activity    : .*(\n|\r\n?)$'
	elif numSection == 1 or numSection==130 :
                expectedInfo ='^     Additional Information   : .*(\n|\r\n?)$'
	elif numSection == 91 :
                expectedInfo ='^       Additional Information : .*(\n|\r\n?)$'
        elif numSection == 85 :
                expectedInfo ='^8.5.[0-9] Other Instrumentation  : .*(\n|\r\n?)$'
        elif numSection == 111 :
                expectedInfo ='     Agency                   : .*(\n|\r\n?)$'
        elif numSection == 112 :
                expectedInfo ='     Mailing Address          : .*(\n|\r\n?)$'
        else :  expectedInfo ='^       Notes                  : .*(\n|\r\n?)$'
...
def parseHeader(lines,begin):
	...
	expected = [
		'^\s*[A-Z0-9]{4}([A-Z0-9]{5})? Site Information Form( \(site log\))?(\n|\r\n?)$',
		'^     (.*)(\n|\r\n?)$',
		'^     See Instructions at:(\n|\r\n?)$',
#		'^     (ftp://igscb.jpl.nasa.gov|https://files.igs.org)/pub/station/general/sitelog_instr.txt(\n|\r\n?)$',
		'^     .*\/pub\/station\/general\/sitelog_instr.txt.*$',
		]
...
def parseForm(lines,begin):
	...
	expected = [
		...
		'^     Report Type              : (NEW|UPDATE)(\n|\r\n?)$',
		'^     If Update:(\n|\r\n?)$',
		'^      Previous Site Log       : (\w{4}([0-9]{2}\w{3})?_[0-9]{8}.log)?',
		#'^      Modified/Added Sections : ([0-9]+|[0-9]+\.[0-9]+\.?[0-9]*,?)*(\n|\r\n?)$',
		'^      Modified/Added Sections : .*(\n|\r\n?)$',
		]
	for i in range(7):
		regexp = re.compile(expected[i])
		resultat = regexp.match(lines[i+begin])
...
def parseSiteID(lines,begin):
	...
	expected = [
		...
		'^       Bedrock Type           : .*(\n|\r\n?)$',
		'^       Bedrock Condition      : .*(\n|\r\n?)$',
		'^       Fracture Spacing       : (none|0 cm|1-10 cm|11-50 cm|51-200 cm|over 200 cm|\(0 cm\/1-10 cm\/11-50 cm\/51-200 cm\/over 200 cm\))?(\n|\r\n?)$',
		'^       Fault zones nearby     : .*(\n|\r\n?)$',
#		'^         Distance/activity    : .*(\n|\r\n?)$',
		]
...
def parseSiteLoc(lines,begin):
	...
	expected = [
		...
		'^       Elevation \(m,ellips\.\)  : (\+|-|)?(\+|-)?([0-9]|\s){0,5}(\.[0-9])?(\n|\r\n?)$',
		]
	Wgs84Coordinates = ""
	for i in range(len(expected)):
		regexp = re.compile(expected[i])
...
#			'^3\.([0-9] |[0-9]{2}) Receiver Type            : (LEICA GR25|TPS GB-1000|TRIMBLE NETRS|TRIMBLE NETR9|TRIMBLE ALLOY|ASHTECH Z-X|ASHTECH UZ-12|ASHTECH Z-XII3|SPECTRA SP90M)(\n|\r\n?)$',
			'^3\.([0-9] |[0-9]{2}) Receiver Type            : .*(\n|\r\n?)$',
#			'^3\.([0-9] |[0-9]{2}) Receiver Type            : (LEICA GR25|TPS GB-1000|TRIMBLE NETRS|TRIMBLE NETR5|TRIMBLE NETR9|TRIMBLE ALLOY|ASHTECH Z-X|ASHTECH UZ-12|ASHTECH Z-XII3|SPECTRA SP90M)(\n|\r\n?)$',
#			'^     Satellite System         : (GPS|GLO|GPS\+GLO|GPS\+GLO\+GAL|GPS\+GLO\+GAL\+BDS\+QZSS\+IRNSS\+SBAS)(\n|\r\n?)$',
		...
		for i in range(len(expected)):
			...
			if not resultat:
				...
				if int(i) == 6:
					data = lines[i+begin].split(": ")[1]
					data = data.rstrip()
					if data in ("CCYY-MM-DDThh:mmZ","(CCYY-MM-DDThh:mmZ)"):
						data = ""
					Removed = "---------------Removed   on  "
					data = data.split("T")[0]
					Removed += data
...
def parseAntenna(lines,begin):
	...
	for i in range(1,numSection+1):
		...
		expected = [
			...
			'^4\.([0-9] |[0-9]{2}) Antenna Type             : .*(\n|\r\n?)$',
			'^     Serial Number            : .*(\n|\r\n?)$',
			'^     Antenna Reference Point  : .*(\n|\r\n?)$',
			'^     Marker->ARP Up Ecc\. \(m\)  : ([0-9]|\s){0,3}\.[0-9]{2,4}(\n|\r\n?)$',
			'^     Marker->ARP North Ecc\(m\) : ([0-9]|\s){0,3}\.[0-9]{2,4}(\n|\r\n?)$',
			'^     Marker->ARP East Ecc\(m\)  : ([0-9]|\s){0,3}\.[0-9]{2,4}(\n|\r\n?)$',
			'^     Alignment from True N    : [0-9]+( deg)?(\n|\r\n?)$',
			'^     Antenna Radome Type      : [0-9A-Z]{4}(\n|\r\n?)$',
			'^     Radome Serial Number     : .*(\n|\r\n?)$',
			'^     Antenna Cable Type       : .*(\n|\r\n?)$',
			'^     Antenna Cable Length     : (([0-9]*\.?[0-9]*) m|\(m\))(\n|\r\n?)$',
			'^     Date Installed           : [0-9]{4}-[0-9]{2}-[0-9]{2}(T[0-9]{2}:[0-9]{2}Z)?(\n|\r\n?)$',
			'^     Date Removed             : ([0-9]{4}-[0-9]{2}-[0-9]{2}(T[0-9]{2}:[0-9]{2}Z)?)|((\()?CCYY-MM-DDThh:mmZ(\))?)(\n|\r\n?)$',
			]
		for i in range(len(expected)):
...
def parseHumidity(lines,begin):
        ...
        for i in range(1,numSection+1):
                ...
                expected = ['^8\.1\.([0-9] |[0-9]{2}) Humidity Sensor Model  : .*(\n|\r\n?)$',
                        ...
                        '^       Calibration date       : ([0-9]{4}-[0-9]{2}-[0-9]{2}|CCYY-MM-DD|\(CCYY-MM-DD\))?(\n|\r\n?)$',
                        '^       Effective Dates        : .*(\n|\r\n?)$',
                        ]
                for i in range(len(expected)):
                        regexp = re.compile(expected[i])
