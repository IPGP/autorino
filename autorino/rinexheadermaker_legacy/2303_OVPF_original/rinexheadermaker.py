...
def parseArgs(arguments):
	try:
		opts, args = getopt.getopt(arguments, "hf:d:", ["help", "file=", "date="])
...
siteLogName, date = parseArgs(sys.argv[1:])
print "siteLogName = " + siteLogName
...
def controleNumeroSection(line,i):
	debut = line
        ...
        if i != 8 and i!=9 and i != 81 and i != 82 and i != 83 and i != 84 and i != 85 and i!=91 and i!=92 and i!=93:
			...
			compteur = 1 
                        while sousSections != []:
                                numSection = int(section[4].strip())
...
def parseComment(lines,begin,numSection):
	#print('Numero de section : %d' % (numSection))
	debutComment = begin
	if numSection == 100 :
	...
	#print('Expected information : %s' % (expectedInfo))
	...
	if numSection <100 :
                ...
                regexp = re.compile(expectedTest)
		#print('DEBUG : %s' % (lines[debutComment]))
                resultat=regexp.search(lines[debutComment])
                begin+=1
        elif numSection ==130:
        ...
        if not resultat :
			...
			#print('DEBUG : resultat is False')
                        expectedSection ='^     Primary Contact$'
                elif numSection ==113:
                        expectedTest ='^     Antenna Graphics with Dimensions$'
                #print('Expected section : %s' % (expectedSection))
                regexp = re.compile(expectedSection)
                #print('Begin before : %d' % (begin))
                while flag == False:
			#print('DEBUG : searching "     Primary Contact" in "%s"' % (lines[begin]))
                        resultat = regexp.search(lines[begin])
			#print(resultat)
                        if not resultat : begin+=1
                        else : flag = True        
                #print('Begin after : %d' % (begin))
                finComment = begin
...
def parseHeader(lines,begin):
	...
	expected = [
		'^     [A-Z0-9]{9} Site Information Form( \(site log\))?(\n|\r\n?)$',
		'^     (.*)(\n|\r\n?)$',
		'^     See Instructions at:(\n|\r\n?)$',
		'^       https://files.igscb.org/pub/station/general/sitelog_instr.txt(\n|\r\n?)$',
		]
...
def parseForm(lines,begin):
	...
	expected = [
		...
		'^     Report Type              : (NEW|UPDATE)(\n|\r\n?)$',
		'^     If Update:(\n|\r\n?)$',
		'^      Previous Site Log       : ([a-z0-9]{9}_[0-9]{8}.log)(\n|\r\n?)|(\(ssssddsss_ccyymmdd.log\)(\n|\r\n?))|(\n|\r\n?)$',
		'^      Modified/Added Sections : (([0-9]+\.[0-9]*\.?[0-9]*,?)*)|(\(n.n,n.n,...\))(\n|\r\n?)$',
		]
	for i in range(7):
                regexp = re.compile(expected[i])
                resultat = regexp.search(lines[i+begin])
...
def parseSiteID(lines,begin):
	...
	expected = [
		...
		'^       Bedrock Type           : (IGNEOUS|METAMORPHIC|SEDIMENTARY|BASALTIC|NONE)|(\(IGNEOUS/METAMORPHIC/SEDIMENTARY\))?(\n|\r\n?)$',
		'^       Bedrock Condition      : (FRESH|JOINTED|WEATHERED)|(\(FRESH/JOINTED/WEATHERED\))?(\n|\r\n?)$',
		'^       Fracture Spacing       : (0 cm|1-10 cm|11-50 cm|51-200 cm|over 200 cm)|(\(0 cm/1-10 cm/11-50 cm/51-200 cm/over 200 cm\))(\n|\r\n?)$',
		'^       Fault zones nearby     : .*(\n|\r\n?)$',
#		'^         Distance/activity    : .*(\n|\r\n?)$',
		]
	for i in range(len(expected)):
...
def parseReceiver(lines,begin):
	...
	for i in range(1,numSection+1):
		...
		expected = [
			'^3\.([0-9] |[0-9]{2}) Receiver Type            : (LEICA GR25|TPS GB-1000|TRIMBLE NETRS|TRIMBLE NETR9|TRIMBLE ALLOY|ASHTECH Z-X|ASHTECH UZ-12|ASHTECH Z-XII3|SEPT POLARX5)(\n|\r\n?)$',
			'^     Satellite System         : (GPS|GLO|GPS\+GLO|GPS\+GLO\+GAL|GPS\+GLO\+GAL\+BDS\+QZSS\+IRNSS\+SBAS|GPS\+GLO\+GAL\+BDS\+QZSS\+SBAS)(\n|\r\n?)$',
			'^     Serial Number            : .*(\n|\r\n?)$',
			'^     Firmware Version         : .*(\n|\r\n?)$',
			'^     Elevation Cutoff Setting : [0-9]* deg(\n|\r\n?)$',
			'^     Date Installed           : [0-9]{4}-[0-9]{2}-[0-9]{2}(T[0-9]{2}:[0-9]{2}Z)?(\n|\r\n?)$',
...
def parseAntenna(lines,begin):
	...
	for i in range(1,numSection+1):
		...
		expected = [
			'^4\.([0-9] |[0-9]{2}) Antenna Type             : (TPSPG_A1(_6)?(\+GP)?|TRM115000\.00|TRM41249\.00|ASH700936A|ASH700228D|ASH700936E|ASH700936E_C|ASH701975\.01A(GP)?|ASH700700\.A|ASH700700\.C|ASH701945C_M|TRM57971\.00|TRM55971\.00|LEIAS10|LEIAR10)[ ]*([A-Z]{4}?)(\n|\r\n?)$',
			'^     Serial Number            : .*(\n|\r\n?)$',
			'^     Antenna Reference Point  : .*(\n|\r\n?)$',
			'^     Marker->ARP Up Ecc\. \(m\)  : [\s0-9]{3}\.[0-9]{4}(\n|\r\n?)$',
			'^     Marker->ARP North Ecc\(m\) : [\s0-9]{3}\.[0-9]{4}(\n|\r\n?)$',
			'^     Marker->ARP East Ecc\(m\)  : [\s0-9]{3}\.[0-9]{4}(\n|\r\n?)$',
			'^     Alignment from True N    : [0-9]+ deg(\n|\r\n?)$',
...
def parseHumidity(lines,begin):
        ...
        for i in range(1,numSection+1):
                ...
                expected = ['^8\.1\.([0-9] |[0-9]{2}) Humidity Sensor Model  : .*(\n|\r\n?)$',
                        '^       Manufacturer           : .*(\n|\r\n?)$',
                        '^       Serial Number          : .*(\n|\r\n?)$',
                        '^       Data Sampling Interval : [\s0-9]* sec(\n|\r\n?)$',
                        '^       Accuracy \(% rel h\)     : (\+|-)?[0-9]*\.?[0-9]*? % rel h(\n|\r\n?)$',
                        '^       Aspiration             : .*(\n|\r\n?)$',
                        '^       Height Diff to Ant     : [0-9]*\.?[0-9]*? m(\n|\r\n?)$',
                        '^       Calibration date       : ([0-9]{4}-[0-9]{2}-[0-9]{2})|(\CCYY-MM-DD\))(\n|\r\n?)$',
                        '^       Effective Dates        : ([0-9]{4}-[0-9]{2}-[0-9]{2}/([0-9]{4}-[0-9]{2}-[0-9]{2}|CCYY-MM-DD))?(\n|\r\n?)$',
                        ]
                for i in range(len(expected)):
                        regexp = re.compile(expected[i])
...
regex = re.compile('^([a-z0-9]{9})_([0-9]{8})\.log$')
#regex = re.compile('^([a-z0-9]+)\.log$')
resultat = regex.search(siteLogName)
if resultat:
...
#				'^     Satellite System         : \(GPS\/GLONASS\/GPS\+GLONASS\)',
	...
	while linesCleaned[nextLine].startswith("4."):
		if linesCleaned[nextLine].startswith("4.x"):
			...
			if not found4X:
				resultat4X = parseAntenna(linesCleaned,nextLine)
				nextLine = resultat4X[0]
                                hEN_m = resultat4X[1]
			else:
