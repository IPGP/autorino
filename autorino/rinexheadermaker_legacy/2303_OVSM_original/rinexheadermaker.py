...
def parseComment(lines,begin,numSection):
	...
	elif numSection == 1 or numSection==113 :
	...
	if numSection <100 :
        ...
        if not resultat :
                ...
                if numSection <100: 
                        ...
                        expectedSection ='^     Antenna Graphics with Dimensions$'
...
def parseHeader(lines,begin):
	...
	expected = [
		'^     [A-Z0-9]{4}([0-9]{2}[A-Z]{3})? Site Information Form( \(site log\))?(\n|\r\n?)$',
		'^     (.*)(\n|\r\n?)$',
		'^     See Instructions at:(\n|\r\n?)$',
		'^       (ftp:\/\/igscb\.jpl\.nasa\.gov|https:\/\/files\.igscb\.org)\/pub\/station\/general\/sitelog_instr\.txt(\n|\r\n?)$',
		]
...
def parseForm(lines,begin):
	...
	expected = [
		...
		'^     Report Type              : (NEW|UPDATE)(\n|\r\n?)$',
		'^     If Update:(\n|\r\n?)$',
		'^      Previous Site Log       : (([a-z0-9]{4}([0-9]{2}[a-z]{3})?_[0-9]{8}\.log)|\(ssssddsss_ccyymmdd\.log\))?(\n|\r\n?)$',
		'^      Modified/Added Sections : ((([0-9]+\.?[0-9]*,?)*)|\(n\.n,n\.n,\.\.\.\))*(\n|\r\n?)$',
		]
	for i in range(7):
                regexp = re.compile(expected[i])
                resultat = regexp.search(lines[i+begin])
...
def parseSiteID(lines,begin):
	...
	expected = [
		...
		'^       Bedrock Type           : (IGNEOUS|METAMORPHIC|SEDIMENTARY|BASALTIC)|(\(IGNEOUS/METAMORPHIC/SEDIMENTARY\))?(\n|\r\n?)$',
...
#		'^       X coordinate \(m\)       : (\+|-)?[0-9]+\.[0-9]+(\n|\r\n?)$',
	expected = [
		...
		'^       Elevation \(m,ellips\.\)  : (\+|-)?[0-9]{1,5}(\.[0-9])?(\n|\r\n?)$',
		]
	Wgs84Coordinates = ""
	for i in range(len(expected)):
		regexp = re.compile(expected[i])
...
def parseReceiver(lines,begin):
	...
	for i in range(1,numSection+1):
		...
		expected = [
			'^3\.([0-9] |[0-9]{2}) Receiver Type            : (SPECTRA SP90M|LEICA GR50|LEICA GR25|TPS GB-1000|TRIMBLE NETRS|TRIMBLE NETR9|ASHTECH Z-X|ASHTECH UZ-12|ASHTECH Z-XII3|TRIMBLE ALLOY|SEPT POLARX5)(\n|\r\n?)$',
			'^     Satellite System         : (GPS|GLO|GPS\+GLO|GPS\+GLO\+GAL|GPS\+GLO\+GAL\+BDS\+QZSS\+SBAS|GPS\+GLO\+GAL\+BDS\+QZSS\+IRNSS\+SBAS|GPS\+GLO\+GAL\+SBAS)(\n|\r\n?)$',
			'^     Serial Number            : .*(\n|\r\n?)$',
			'^     Firmware Version         : .*(\n|\r\n?)$',
			'^     Elevation Cutoff Setting : [0-9]*( deg)?(\n|\r\n?)$',
			'^     Date Installed           : [0-9]{4}-[0-9]{2}-[0-9]{2}(T[0-9]{2}:[0-9]{2}Z)?(\n|\r\n?)$',
			'^     Date Removed             : ([0-9]{4}-[0-9]{2}-[0-9]{2}(T[0-9]{2}:[0-9]{2}Z)?)|(\(CCYY-MM-DDThh:mmZ\))|(CCYY-MM-DDThh:mmZ)(\n|\r\n?)$',
			'^     Temperature Stabiliz\.    : ([0-9]*|NONE|none)(\n|\r\n?)$',
			]
		for i in range(len(expected)):
			regexp = re.compile(expected[i])
			resultat = regexp.search(lines[i+begin])
...
def parseAntenna(lines,begin):
	...
	for i in range(1,numSection+1):
		...
		expected = [
			'^4\.([0-9] |[0-9]{2}) Antenna Type             : (TPSPG_A1(_6)?(\+GP)?|TRM41249\.00|ASH700936A|ASH700228D|ASH700936E|ASH700936E_C|ASH701975\.01A(GP)?|ASH700700\.A|ASH700700\.C|ASH701945C_M|TRM57971\.00|TRM55971\.00|LEIAS10|ASH700718B|LEIAR10|AERAT1675_120|TRM115000\.00)[ ]*([A-Z]{4}?)(\n|\r\n?)$',
			'^     Serial Number            : .*(\n|\r\n?)$',
			'^     Antenna Reference Point  : .*(\n|\r\n?)$',
			'^     Marker->ARP Up Ecc\. \(m\)  : [ 0-9]{1,3}\.[0-9]{4}(\n|\r\n?)$',
			'^     Marker->ARP North Ecc\(m\) : [ 0-9]{1,3}\.[0-9]{4}(\n|\r\n?)$',
			'^     Marker->ARP East Ecc\(m\)  : [ 0-9]{1,3}\.[0-9]{4}(\n|\r\n?)$',
			'^     Alignment from True N    : [0-9]+( deg)?(\n|\r\n?)$',
			'^     Antenna Radome Type      : [0-9A-Z]{4}(\n|\r\n?)$',
			'^     Radome Serial Number     : .*(\n|\r\n?)$',
			'^     Antenna Cable Type       : .*(\n|\r\n?)$',
			'^     Antenna Cable Length     : [0-9]* m(\n|\r\n?)$',
			'^     Date Installed           : [0-9]{4}-[0-9]{2}-[0-9]{2}(T[0-9]{2}:[0-9]{2}Z)?(\n|\r\n?)$',
			'^     Date Removed             : ([0-9]{4}-[0-9]{2}-[0-9]{2}(T[0-9]{2}:[0-9]{2}Z)?)|(\(CCYY-MM-DDThh:mmZ\))|(CCYY-MM-DDThh:mmZ)(\n|\r\n?)$',
			]
		for i in range(len(expected)):
...
def parseMoreInfo(lines,begin):
	...
	for i in range(len(expected)):
		...
		if not resultat:
			...
			sys.exit()
	begin = begin + len(expected)
	begin = parseComment(lines,begin,113)
...
regex = re.compile('^([a-z0-9]{4})([0-9]{2}[a-z]{3})?_([0-9]{8})\.log$')
...
#				'^     Satellite System         : \(GPS\/GLONASS\/GPS\+GLONASS\)',
				'^     Satellite System         : \(GPS\+GLO\+GAL\+BDS\+QZSS\+SBAS\)|\(GPS\+GLO\+GAL\+BDS\+QZSS\+IRNSS\+SBAS\)',
